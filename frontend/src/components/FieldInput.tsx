...
interface Props {
  ...
  options?: Value[]; disabled?: boolean; label: string; translateOption?: (value: Value) => string
}
export function FieldInput({id, value, onChange, type, options, disabled, label, translateOption}: Props) {
  ...
  if (type === 'textarea' || type === 'task_priority') return <textarea id={id} value={String(value ?? '')} rows={4} disabled={disabled} onChange={event => onChange(event.target.value)} />
